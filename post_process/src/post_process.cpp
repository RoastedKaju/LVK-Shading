...
static int meshDataIndex = 1;
static bool showWireframe = false;
static bool autoRotateMesh = true;
static float baseColor[3] = { 0.3f, 0.5f, 0.1f };
...
static float chromaticAberrationStrength = 0.005f;
...
)
{
	...
	ImGui::SliderFloat("Chromatic Aberration", &chromaticAberrationStrength, 0.0f, 0.02f);
	ImGui::End();
...
int main()
{
...

	{
		...
		lvk::Holder<lvk::ShaderModuleHandle> ppVert = loadShaderModule(ctx, std::filesystem::absolute(SHADER_DIR"/post_process.vert"));
		lvk::Holder<lvk::ShaderModuleHandle> ppFrag = loadShaderModule(ctx, std::filesystem::absolute(SHADER_DIR"/post_process.frag"));
		...
		// Off-Screen texture
		// This is the render target where all of our scene information will be written to
		...
		offscreenDesc.usage = lvk::TextureUsageBits_Attachment | lvk::TextureUsageBits_Sampled; // Will be sampled in pass 2
		offscreenDesc.debugName = "Offscreen Color";
		...
		lvk::RenderPipelineDesc ppDesc{};
		// No vertexInput — fullscreen triangle is generated in the vertex shader
		ppDesc.smVert = ppVert;
		ppDesc.smFrag = ppFrag;
		ppDesc.color[0].format = ctx->getSwapchainFormat();
		// No depth attachment needed for a fullscreen post-process pass
		lvk::Holder<lvk::RenderPipelineHandle> postProcessPipeline = ctx->createRenderPipeline(ppDesc);

		LVK_ASSERT(soildPipeline.valid());
		LVK_ASSERT(wireframePipeline.valid());
		LVK_ASSERT(postProcessPipeline.valid());
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			scenePass.color[0].storeOp = lvk::StoreOp_Store; // keep result!
			...
			lvk::Framebuffer offscreenFB;
			offscreenFB.color[0].texture = offscreenTexture; // Off-Screen texture
			offscreenFB.depthStencil.texture = depthTexture;
			...
			uniformData.lightingParams = glm::vec4(specularStrength, vignetteStrength, chromaticAberrationStrength, (float)glfwGetTime());
			uniformData.textureId = offscreenTexture.index();

			// Command buffer
			lvk::ICommandBuffer& buff = ctx->acquireCommandBuffer();
			buff.cmdUpdateBuffer(uniformBuffer, uniformData);
			// Begin Rendering
			buff.cmdBeginRendering(scenePass, offscreenFB);
			...
			// 2nd-Pass
			lvk::RenderPass ppPass;
			ppPass.color[0].loadOp = lvk::LoadOp_DontCare; // we overwrite everything

			lvk::Framebuffer swapchainFB;
			swapchainFB.color[0].texture = ctx->getCurrentSwapchainTexture();
			// no depth needed

			buff.cmdBeginRendering(ppPass, swapchainFB);
			buff.cmdPushDebugGroupLabel("Post Process Pass", 0x00ff00ff);

			buff.cmdBindRenderPipeline(postProcessPipeline);
			buff.cmdBindDepthState({ .compareOp = lvk::CompareOp_AlwaysPass, .isDepthWriteEnabled = false });
			buff.cmdDraw(3);

			// UI
			showUI(*imguiCtx, swapchainFB, buff);
